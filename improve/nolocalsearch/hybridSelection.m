...
%混合群体选择：均匀选择
%混合群体大小为K
PopulationTs_hybrid = zeros(K/2,numOfSubTasks);
PopulationFs_hybrid = zeros(K/2,numOfSubTasks);
PopulationMs_hybrid = zeros(K/2,numOfSubTasks);
for i = 1:K/4
    num1 = randi(K);
    num2 = randi(K);
    ...
    if fitness_3 >= fitness_4
        PopulationTs_hybrid(K/4 + i,:) = PopulationTs_ordinary(num4,:);
        PopulationFs_hybrid(K/4 + i,:) = PopulationFs_ordinary(num4,:);
        PopulationMs_hybrid(K/4 + i,:) = PopulationMs_ordinary(num4,:);
    else
        PopulationTs_hybrid(K/4 + i,:) = PopulationTs_ordinary(num3,:);
        PopulationFs_hybrid(K/4 + i,:) = PopulationFs_ordinary(num3,:);
        PopulationMs_hybrid(K/4 + i,:) = PopulationMs_ordinary(num3,:);
    end
end
end
