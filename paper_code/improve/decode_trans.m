function [start_Time_pop,end_Time_pop,start_Time_T_pop,end_Time_T_pop] = decode_trans(numOfTasks,numOfSubTasks,vectormcellPerFactory,numOfTotalMcell,vectorNumSubTasks,vectorSumOfSubTasks,numOfPopulation,PopulationTs,PopulationFs,PopulationMs,tableOfManuTime,tableOfTransTime)
%   解码函数：顺序解码与插入式解码
%   输入：
%   输出：工厂顺序矩阵F_S、加工单元顺序矩阵M_S、加工时间顺序矩阵M_T、每个加工单元上的子任务集合、每个加工单元的完工时间以及种群中各个子任务所对应的工厂、加工单元和加工时间。

%步骤1：对工厂选择序列进行解码，从左到右依次读取并转换成工厂顺序矩阵F_S；对加工单元选择序列进行解码，从左到右依次读取并转换成加工单元顺序矩阵M_S和加工时间顺序矩阵M_T。

%步骤2：计算每个加工单元分配到的子任务集合,并进行插入式解码。
...
for pop = 1:numOfPopulation
    ...
    for i = 1:numOfTasks
        ...
        for j = 1:numSubT
            ...
            if time ~= 0
                ...
                A = [vectormcellPerFactory,numOfTotalMcell];
                for p = 1:num_fac
    ...
    for k = 1:numOfSubTasks
        ...
        factoryChoose_Time = transTime(taskId,count);
...
%           [~,ind] = find(mcell == mcell_max_end_time);
        ...
        if count <= 1
            ...
            front_subtask_end_time0 = time(time ~= 0);
            front_subtask_end_time = front_subtask_end_time0 + factoryChoose_Time;
        end
        ...
        [end_time2,Y] = sort(end_time1);
        start_time2 = start_time1(Y);
        ...
        if num_free == 0
            ...
            judge = zeros(1,num_free);
            for m = 1:num_free
                current_free_start = free_start(1,free_loc(1,m));
                current_free_end = free_end(1,free_loc(1,m));
        ...
        elseif (count > 1) && (L == 0)%有前置子任务，当前加工单元空闲
            ...
            start_Time_T(loc,subtaskId) = front_subtask_end_time - factoryChoose_Time;
            end_Time_T(loc,subtaskId) = front_subtask_end_time;
        ...
        elseif (count > 1) && (L ~= 0) && (num_free == 0) && (front_subtask_end_time <= mcell_max_end_time)%有前置子任务，当前加工单元繁忙，且无空闲时间段，前置子任务结束时间小于机器空闲时间
            start_Time(loc,subtaskId) = mcell_max_end_time;
            end_Time(loc,subtaskId) = mcell_max_end_time + mcellChoose_Time;
            start_Time_T(loc,subtaskId) = mcell_max_end_time - factoryChoose_Time;
            end_Time_T(loc,subtaskId) = mcell_max_end_time;
        elseif (count > 1) && (L ~= 0) && (num_free ~= 0) && (judge_insert == 1) && (front_subtask_end_time <= mcell_max_end_time)%有前置子任务，当前加工单元繁忙，且有空闲时间段，但不满足插入条件
            start_Time(loc,subtaskId) = mcell_max_end_time;
            end_Time(loc,subtaskId) = mcell_max_end_time + mcellChoose_Time;
            start_Time_T(loc,subtaskId) = mcell_max_end_time - factoryChoose_Time;
            end_Time_T(loc,subtaskId) = mcell_max_end_time;
        ...
        elseif (count > 1) && (L ~= 0) && (num_free ~= 0) && (judge_insert == 0) && (free_start(free_loc(insert)) >= front_subtask_end_time)%有前置子任务，当前加工单元繁忙，且有空闲时间段，满足插入条件,并且空闲开始时间大于前置子任务的完成时间
            start_Time(loc,subtaskId) = free_start(free_loc(insert));
            end_Time(loc,subtaskId) = free_start(free_loc(insert)) + mcellChoose_Time;
            start_Time_T(loc,subtaskId) = free_start(free_loc(insert)) - factoryChoose_Time;
            end_Time_T(loc,subtaskId) = free_start(free_loc(insert));
