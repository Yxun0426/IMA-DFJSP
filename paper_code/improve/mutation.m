function [PopulationTs_new1,PopulationFs_new1,PopulationMs_new1] = mutation(PopulationTs_new,PopulationFs_new,PopulationMs_new,P_m,numOfTasks,numOfPopulation,mcellPerFactory,vectorNumSubTasks,vectorSumOfSubTasks,vectormcellPerFactory,tableOfMcellOptional,tableOfFactoryOptional)
%变异操作
...
for i = 1:numOfPopulation
    ...
    for j = 1:numOfTasks
        ...
        for m = 1:num
            ...
            P = rand(1);
            if P <= P_m
                [F_list,ind1] = find(tableOfFactoryOptional(count + m,:) ~= 0);
...
%3.任务排序序列：
%邻域变异算子
for k = 1:numOfPopulation/2
    TS_new_ind = PopulationTs_new(k,:);
    TS_new_ind1 = PopulationTs_new(k,:);
    P1 = rand(1);
    if P1 <= P_m
        index0 = zeros(1,3);
        list = randperm(length(TS_new_ind));%随机选取序列上的位置，并返回索引
        index0(1,1) = list(1);
        a = TS_new_ind(list(1));
        ind = find(TS_new_ind(list) ~= a,1);
        index0(1,2) = ind;
        b = TS_new_ind(list(ind));
        ind1 = find(TS_new_ind(list) ~= a & TS_new_ind(list) ~= b,1);
        index0(1,3) = ind1;
        index_list = perms(index0);
        P2 = randi(6);
        index_current = index_list(P2,:);
        for m = 1:3
            TS_new_ind1(1,index0(1,m)) = TS_new_ind(1,index_current(1,m));
        end
    else
...
%交换变异算子
for k = numOfPopulation/2 + 1:numOfPopulation
    TS_new_ind = PopulationTs_new(k,:);
    TS_new_ind1 = PopulationTs_new(k,:);
    P1 = rand(1);
    if P1 <= P_m
        index1 = zeros(1,2);
        list1 = randperm(length(TS_new_ind));%随机选取序列上的位置，并返回索引
        index1(1,1) = list1(1);
        a = TS_new_ind(list1(1));
        ind = find(TS_new_ind(list1) ~= a,1);
        index1(1,2) = ind;
        TS_new_ind1(1,index1(1,1)) = TS_new_ind(1,index1(1,2));
        TS_new_ind1(1,index1(1,2)) = TS_new_ind(1,index1(1,1));
        TS_new_ind = TS_new_ind1;
    else
        TS_new_ind = TS_new_ind1;
    end
