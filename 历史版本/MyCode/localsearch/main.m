...
for iter = 1:numOfIterate-1
    ...
    [PopulationTs_elite_new2,PopulationFs_elite_new2,PopulationMs_elite_new2] = mutation(PopulationTs_elite_new1,PopulationFs_elite_new1,PopulationMs_elite_new1,P_m,numOfTasks,K,mcellPerFactory,numOfSubTasks,vectorNumSubTasks,vectorSumOfSubTasks,vectormcellPerFactory,tableOfMcellOptional,tableOfTransTime,tableOfManuTime);
    %解码与评价
    [~,~,~,~,start_Time_pop_elite,end_Time_pop_elite,start_Time_trans_elite,end_Time_trans_elite] = decode(numOfTasks,numOfSubTasks,vectormcellPerFactory,numOfTotalMcell,vectorNumSubTasks,vectorSumOfSubTasks,K,PopulationTs_elite_new2,PopulationFs_elite_new2,PopulationMs_elite_new2,tableOfTransTime,tableOfManuTime);
    [makespan_population_elite,makespan_best_elite,index_best_elite,index_Population_elite] = fitness(end_Time_trans_elite,K);
    ...
    [PopulationTs_ordinary_new2,PopulationFs_ordinary_new2,PopulationMs_ordinary_new2] = mutation(PopulationTs_ordinary_new1,PopulationFs_ordinary_new1,PopulationMs_ordinary_new1,P_m,numOfTasks,K,mcellPerFactory,numOfSubTasks,vectorNumSubTasks,vectorSumOfSubTasks,vectormcellPerFactory,tableOfMcellOptional,tableOfTransTime,tableOfManuTime);
    %解码与评价
    [~,~,~,~,start_Time_pop_ordinary,end_Time_pop_ordinary,start_Time_trans_ordinary,end_Time_trans_ordinary] = decode(numOfTasks,numOfSubTasks,vectormcellPerFactory,numOfTotalMcell,vectorNumSubTasks,vectorSumOfSubTasks,K,PopulationTs_ordinary_new2,PopulationFs_ordinary_new2,PopulationMs_ordinary_new2,tableOfTransTime,tableOfManuTime);
    [makespan_population_ordinary,makespan_best_ordinary,index_best_ordinary,index_Population_ordinary] = fitness(end_Time_trans_ordinary,K);
    ...
    %解码与评价
    [~,~,~,~,start_Time_pop_hybrid,end_Time_pop_hybrid,start_Time_trans_hybrid,end_Time_trans_hybrid] = decode(numOfTasks,numOfSubTasks,vectormcellPerFactory,numOfTotalMcell,vectorNumSubTasks,vectorSumOfSubTasks,numOfPopulation-K,PopulationTs_hybrid_new1,PopulationFs_hybrid_new1,PopulationMs_hybrid_new1,tableOfTransTime,tableOfManuTime);
    [makespan_population_hybrid,makespan_best_hybrid,index_best_hybrid,index_Population_hybrid] = fitness(end_Time_trans_hybrid,numOfPopulation-K);
    %序列合并
    ...
    %%%解码
    [~,~,~,~,start_Time_pop_merge,end_Time_pop_merge,start_Time_trans_merge,end_Time_trans_merge] = decode(numOfTasks,numOfSubTasks,vectormcellPerFactory,numOfTotalMcell,vectorNumSubTasks,vectorSumOfSubTasks,numOfPopulation,PopulationTs_merge1,PopulationFs_merge1,PopulationMs_merge1,tableOfTransTime,tableOfManuTime);
    ...
    %%局部搜索
    [TS_set,FS_set,MS_set,L] = localSearch(numOfTasks,numOfSubTasks,PopulationTs_merge1,PopulationFs_merge1,PopulationMs_merge1,index_best_merge,index_Population_merge,tableOfManuTime,vectorNumSubTasks,vectorSumOfSubTasks);
    MK = zeros(1,L);
    ...
    LJ = length(J);
    if LJ == 0
        PopulationTs_update = PopulationTs_merge1;
        PopulationFs_update = PopulationFs_merge1;
        PopulationMs_update = PopulationMs_merge1;
        ...
        PopulationTs_merge1(index_best_merge,:) = TS_set(J,:);
        PopulationFs_merge1(index_best_merge,:) = FS_set(J,:);
        PopulationMs_merge1(index_best_merge,:) = MS_set(J,:);
        PopulationTs_update = PopulationTs_merge1;
        PopulationFs_update = PopulationFs_merge1;
        PopulationMs_update = PopulationMs_merge1;
