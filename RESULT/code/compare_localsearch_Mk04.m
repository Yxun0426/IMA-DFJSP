...
x = 1:1:100;
load('localsearch.mat');
load('Nolocalsearch.mat');
plot(x,makespan_best_T_iter,'-ko',x,makespan_best_T_iter_no,'-kx');
title('在数据集DMk04上的优化过程迭代曲线','FontName','黑体','FontSize',14);
axis( [0,110,75,110])  %确定x轴与y轴框图大小
set(gca,'XTick',[0:10:110]) %x轴范围
set(gca,'YTick',[75:5:110]) %y轴范围
...
imwrite(img,'../结果图/localsearch.png'); % 保存到工作目录下，名字为"a.png"