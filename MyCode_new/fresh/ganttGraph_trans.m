function ganttGraph_trans(numOfFactory,mcellPerFactory,vectormcellPerFactory,numOfTasks,numOfSubTasks,numOfTotalMcell,makespan_best,start_Time_best,end_Time_best,start_Time_T_best,end_Time_T_best,vectorSumOfSubTasks)
%绘制甘特图
...
for a = 1:numOfTotalMcell
    ...
    for b = 1:L
        ...
        rec2 = zeros(1,4);
        rec2(1,1) = current_starttime_T;
        rec2(1,2) = a - 0.5;
        rec2(1,3) = duration_time_T;
        rec2(1,4) = 0.5;
        txt2=sprintf('');%将工序号，加工时间连成字符串
        if duration_time_T ~= 0
            rectangle('Position',rec2,'LineWidth',0.5,'LineStyle','-','FaceColor',color(taskId,:));
        end
        text(current_starttime_T,a-0.25,txt2,'FontSize',8,'FontWeight','Bold','FontName','Times New Roman');%字体的坐标和其它特性
    end
end
end