function [TS,FS,MS] = randomInit(numOfSubTasks,vectorSubTaskSequence,numOfFactory,tableOfFactoryOptional,mcellPerFactory,tableOfMcellOptional,vectormcellPerFactory)
%   种群初始化方法：随机初始化策略
...
for m=1:numOfSubTasks
    factories=tableOfFactoryOptional(m,:);
...
for i=1:numOfSubTasks
    factoryID=FS(i);
    numMcells=mcellPerFactory(1,factoryID);%mcellPerFactory由数据集读取时生成,表示每个工厂具有的加工单元数量。
    num = vectormcellPerFactory(1,factoryID);
